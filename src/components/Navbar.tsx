...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      {/* Mobile navigation */}
      <nav className={`lg:hidden fixed inset-0 bg-darker/95 backdrop-blur-md flex flex-col items-center justify-center space-y-8 text-xl transform transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        {/* Close button */}
        <button 
          className="absolute top-4 right-4 text-text-light p-2" 
          onClick={toggleMenu}
        ...
        <Button 
          ...
          asChild
        >
          <a href="/CV_Mateo_Kodra.pdf" download="CV_Mateo_Kodra.pdf" className="flex items-center gap-2" onClick={toggleMenu}>
            <Download size={16} />
            Resume
          </a>
        </Button>
